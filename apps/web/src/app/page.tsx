...
export default function Home() {
  ...
  const handleSubmit = async () => {
    const response = await fetch('http://localhost:3333/new-question', {
      method: 'POST',
      headers: {
  ...
  return (
    <div className="flex flex-col items-center gap-6 pb-[150px] px-28 font-[family-name:var(--font-geist-sans)]">
      {chat.map((item, index) => (
        <div
          key={index}
          className="bg-zinc-700 p-4 rounded-lg text-white text-center"
        >
          {item.message}
        </div>
      ))}
      ...
      <div className="fixed bottom-6 mx-auto bg-zinc-700 flex items-center justify-center p-4 rounded-lg text-white text-center gap-2">
        <textarea
          ...
          className="w-[600px] border border-zinc-900 rounded p-2 resize-none"
        />
